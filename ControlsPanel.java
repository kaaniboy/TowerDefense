...
public class ControlsPanel extends JPanel implements ActionListener {
	...
	//Instance fields for the various labels and buttons used in the ControlsPanel.
	...
	public ControlsPanel() {
		...
		//Create the label showing money and set it to be centered horizontally.
		...
		//buttonsPanel uses a GridLayout to display two upgrade buttons side-by-side.
		...
		increaseSpeedButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//If there is a selected turret and the user has enough money, upgrade the turret's speed.
				if (selected != null
						&& Game.getMoney() >= selected.getPriceToUpgradeSpeed()
						&& selected.getSpeed() > 0) {
					//Reduce how much money the user has remaining after the purchase.
		...
		increaseDamageButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Turret selected = gamePanel.getSelectedTurret();
				//If there is a selected turret and the user has enough money, upgrade the turret's damage.
				if (selected != null
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (!Game.isDone()) {
			...
			//If the user has selected a turret, make the upgrades panel usable.
			if (gamePanel.getSelectedTurret() != null) {
				...
				//If the user has not selected a turret, make the upgrades panel unusable.
			...
			//If the game has already ended, disable all buttons and hide all labels.
			increaseSpeedButton.setEnabled(false);
			increaseDamageButton.setEnabled(false);
			infoLabel.setVisible(false);
			damageLabel.setVisible(false);
